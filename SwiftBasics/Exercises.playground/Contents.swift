...
let firstIndex = Int(arc4random() % 9)
let secondIndex = Int(arc4random() % 9)
let thirdIndex = Int(arc4random() % 9)

let yourSentence = "I want to see the \(nounArray[2]) while drinking \(nounArray[7])."
let yourSillySentence = "I want to see the \(nounArray[firstIndex]) while sitting along with the \(nounArray[secondIndex]) at the \(nounArray[thirdIndex])."
//: ### Exercise 2
...
let myNewString = "HEY! " + didYouKnowString.uppercased()
//: ### Exercise 3
...
//desired output = "$5.67"
let josieMoney = "$\(numOfPennies/100).\(numOfPennies%100)"
//: # Let or Var?
import UIKit
import Foundation
//: ### Exercise 9
